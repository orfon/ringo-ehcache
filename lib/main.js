...
CacheManager.prototype.getCache = function(name) {
   ...
   if (cache == null) {
      throw "CacheManager.getCache(): Unknown cache '" + name +
...
CacheManager.prototype.get = function(cacheName, key) {
   var element = this.getCache(cacheName).get(toJava(key));
   if (element != null) {
...
CacheManager.prototype.getElement = function(cacheName, key) {
   return this.getCache(cacheName).get(toJava(key));
};
...
CacheManager.prototype.removeElement = function(cacheName, key) {
   return this.getCache(cacheName).remove(toJava(key));
};
